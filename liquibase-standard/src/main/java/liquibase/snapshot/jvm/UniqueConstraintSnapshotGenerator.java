    ...
    protected List<Map<String, Object>> listColumns(UniqueConstraint example, Database database, DatabaseSnapshot snapshot) throws DatabaseException {
        ...
        if (columnCache == null) {
            ...
            if ((database instanceof MySQLDatabase) || (database instanceof HsqlDatabase)) {
                ...
                List<Map<String, Object>> rows = Scope.getCurrentScope().getSingleton(ExecutorService.class).getExecutor("jdbc", database).queryForList(new RawParameterizedSqlStatement(sql.toString()));
            ...
            List<Map<String, Object>> rows = Scope.getCurrentScope().getSingleton(ExecutorService.class).getExecutor("jdbc", database).queryForList(new RawParameterizedSqlStatement(rawSql, parameters.toArray()));
